...
function Storefront() {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Auto-clear cart if checkout was successful
      if (data.content && data.content.includes('mock payment was processed successfully')) {
        clearCart();
      }
  ...
  useEffect(() => {
    ...
    if (query) {
      trackEvent('search', { query, category: searchParams.get('category') || 'All' });
    }
  }, [searchParams]);
  ...
  return (
    <>
      <div className="flex max-w-[1920px] mx-auto min-h-screen pt-28 relative z-10 bg-transparent">
